...
// Get model parameters from the custom composable
const { selectedModel, parameters } = useModelTraining();

const sendParameters = async () => {
  try {
    // Create the correct structure for the parameters
    const requestData = {
      general: {
      ...
      model_specific: {
        validationSplit: parameters.validationSplit,
        hiddenLayers: parameters.hiddenLayers,
        activation: parameters.activation,
      },
    };
    // Send parameters to backend
    const response = await axios.post('http://127.0.0.1:8000/api/parameters/', requestData);
    console.log('Parameters sent successfully:', response.data);
    // Emit the event to navigate to the "train" step after the request is successful
    emit('navigate', 'train');
  } catch (error) {
    console.error('Error sending parameters:', error.response || error);
  }
};
</script>
